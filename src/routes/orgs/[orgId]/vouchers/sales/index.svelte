<script lang="ts">
    ...
    const creditMainNodes = [11]
    const debitMainNodes = [28, 23, 21, 18]
    const initialTrxType = 'debit'
    const newAddType = 'credit'
	...
	$: {
        ...
        $formData.journals[0].amount = journalCreditTotal;
    }
...
<div class="max-w-7xl mx-auto">
	<div class="flex flex-col gap-4 m-4 max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-xl p-4">
        ...
        <h2 class="text-xl">Select Template</h2>
        <div class="flex flex-row gap-4 p-4 justify-center">
            <button class="btn" on:click={() => selectedTemplate = 'simple'}>Simple</button>
            <button class="btn" on:click={() => goto(`/orgs/${organizationId}/vouchers/trips`)}>Trip Booking</button>
        </div>
                        ...
                        {#if j === $formData.journals.length - 1}
                            <div>
                                <button
                                    ...
                                    type="button"
                                    on:click={() => add(formData, journalDiffAbs, 'credit')}>{addJournalButtonText}</button
                                >
                            </div>
                            {#if roundOff}
                                <div>
                                    <button
                                        class="{journalDiffIsCredit ? 'success-btn' : 'danger-btn'} mx-auto"
                                        type="button"
                                        on:click={() => add(formData, journalDiffAbs, 'credit')}>Add roundoff {journalDiffAbs}</button
                                    >
                                </div>
                            {/if}
                        {/if}
